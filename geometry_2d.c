#ifndef geometry_2d_c
#define geometry_2d_c
...
{
    return(point.x >= aab.min.x && point.x <= aab.max.x &&
           point.y >= aab.min.y && point.y <= aab.max.y);
}
                      ...
                      struct v2 position)
{
    ...
    bool32 result = 0;
    for (int32 index = 0; index < number_of_vertices; index += 3)
    {
...
{
    return
        _box2_circle_intersection(box.position,
                                  box.half_extend,
                                  box.rotation,
                                  circle.position,
                                  circle.radius,
                     ...
                     enum box2_edge_number clip_edge)
{
    ...
    if (distance_0 <= 0.0f) out[number_of_points++] = in[0];
    if (distance_1 <= 0.0f) out[number_of_points++] = in[1];
...
{
    if (a.x == b.x && a.y == b.y) return length_v2(sub_v2(a, point));
